    ...
    *,
    ...
    if article is not None:
        article_payload = {
            "slug": article.slug,
            "title": article.title,
            "kind": getattr(article, "kind", "Article") or "Article",
            "topic": getattr(article, "topic", None) or getattr(article, "category", None) or "Uncategorized",
            "published_date": str(getattr(article, "published_iso", "") or "")[:10] or None,
        }
