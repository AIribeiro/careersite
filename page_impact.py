...
import html
from site_assets import AI_PANEL_URI, IMPACT_KEYNOTE_URI
...
def impact() -> str:
    ...
    practice = f'''<section class="section white"><div class="container speak"><img src="{AI_PANEL_URI}" alt="Jair Ribeiro discussing AI with senior industry peers" loading="lazy" decoding="async"><div><p class="eyebrow">Leadership in practice</p><h2>Enterprise AI moves through conversation before it moves through technology.</h2><p>Much of the leadership work happens before a formal decision: getting business owners, technical specialists and governance stakeholders to describe the same problem in compatible language. That is where assumptions surface, ownership becomes visible and a promising idea either earns a credible next step or stops consuming attention.</p><p>The purpose of those conversations is not alignment for its own sake. It is better decision quality — enough shared understanding to make trade-offs explicit, assign ownership and move with confidence.</p></div></div></section>'''

    keynote = f'''<section class="section soft"><div class="container speak"><img src="{IMPACT_KEYNOTE_URI}" alt="Jair Ribeiro speaking to a large enterprise audience" loading="lazy" decoding="async"><div><p class="eyebrow">Leading at scale</p><h2>Adoption becomes real when a broad audience can connect AI to its work.</h2><p>Enterprise capability is not created by a central team alone. Leaders have to make the subject understandable enough for business owners, practitioners and specialists to recognize where AI is useful, where its limits matter and what responsible use requires. Speaking to large internal and external audiences has been one part of that broader adoption work.</p></div></div></section>'''
