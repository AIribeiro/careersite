...
.featured-thinking .kicker,.decision-note .kicker,.format-card span,.article-meta,.article-aside span,.recent-card .kicker{font-size:9px;font-weight:900;letter-spacing:.14em;text-transform:uppercase}
...
@media(max-width:980px){.thinking-now,.article-layout{grid-template-columns:1fr}.format-strip,.recent-grid{grid-template-columns:repeat(2,1fr)}.article-aside{position:static}.featured-thinking{min-height:360px}}
@media(max-width:700px){.format-strip,.recent-grid{grid-template-columns:1fr}.featured-thinking,.decision-note{padding:24px}.article-hero{padding:58px 0 48px}.article-body p{font-size:15px}.article-body h2{font-size:30px}}
</style>'''
...
def render_long(kind: str, title: str, standfirst: str, date: str, body: str, aside: str) -> str:
    return f'''{THINKING_CSS}{nav("thinking")}<main><section class="article-hero"><div class="container"><a class="article-back" href="?page=thinking" target="_self">← Back to Thinking</a><div class="article-meta">{kind}</div><h1>{title}</h1><p class="standfirst">{standfirst}</p><div class="article-date">{date}</div></div></section><section class="section paper"><div class="container article-layout"><article class="article-body">{body}</article><aside class="article-aside">{aside}</aside></div></section>{opportunity()}</main>{footer()}'''


def render_note(kind: str, title: str, standfirst: str, date: str, note_title: str, body: str) -> str:
    return f'''{THINKING_CSS}{nav("thinking")}<main><section class="article-hero"><div class="container"><a class="article-back" href="?page=thinking" target="_self">← Back to Thinking</a><div class="article-meta">{kind}</div><h1>{title}</h1><p class="standfirst">{standfirst}</p><div class="article-date">{date}</div></div></section><section class="section paper"><div class="container"><article class="note-full"><h2>{note_title}</h2><p>{body}</p></article></div></section>{opportunity()}</main>{footer()}'''
